...
func (s *APIServer) validateConfigs(req upsertBotConfigRequest) *upsertBotConfigResponseErrors {
	...
	if _, e := strkey.Decode(strkey.VersionByteSeed, req.TraderConfig.TradingSecretSeed); e != nil {
		...
		if e != nil {
			...
			if req.TraderConfig.IsTradingSdex() && req.TraderConfig.HorizonURL == s.apiTestNet.HorizonURL {
				...
				if e != nil {
					...
					case *horizonclient.Error:
						if t.Problem.Status == 404 || strings.ToLower(t.Problem.Title) == "resource missing" {
							...
							log.Printf("error from horizon while validating secret key being used on test network: status = %d, message = %s - %s", t.Problem.Status, t.Problem.Title, t.Problem.Detail)
