...
func TestMakeValid(t *testing.T) {
	...
	_, e := MakeInitializedCcxtExchange("kraken", api.ExchangeAPIKey{}, []api.ExchangeHeader{})
	if e != nil {
...
func TestMakeInvalid(t *testing.T) {
	...
	_, e := MakeInitializedCcxtExchange("missing-exchange", api.ExchangeAPIKey{}, []api.ExchangeHeader{})
	if e == nil {
...
func TestFetchTickers(t *testing.T) {
	...
	c, e := MakeInitializedCcxtExchange("binance", api.ExchangeAPIKey{}, []api.ExchangeHeader{})
	if e != nil {
...
func runTestFetchOrderBook(k orderbookTest, t *testing.T) {
	...
	c, e := MakeInitializedCcxtExchange(k.exchangeName, api.ExchangeAPIKey{}, []api.ExchangeHeader{})
	if e != nil {
		assert.Fail(t, fmt.Sprintf("error when making ccxt exchange: %s", e))
		return
	}
...
func TestFetchMyTrades(t *testing.T) {
	...
	for _, k := range []struct {
		...
		tradingPairString := strings.Replace(k.tradingPair, "/", "_", -1)
		t.Run(fmt.Sprintf("%s-%s", k.exchangeName, tradingPairString), func(t *testing.T) {
			c, e := MakeInitializedCcxtExchange(k.exchangeName, k.apiKey, []api.ExchangeHeader{})
			if e != nil {
				assert.Fail(t, fmt.Sprintf("error when making ccxt exchange: %s", e))
				return
			}
